...
*/
locals {
  admin_frontend_env_vars = [
    { "name" : "DM_APP_NAME", "value" : local.service_name_admin_frontend },
...
module "admin_frontend_service" {
  source = "../../modules/balanced-ecs-service"

  aws_region         = var.aws_region
  aws_target_account = var.aws_target_account
  # No 'free access' routes in this service so no container_healthcheck_command 😔
  container_memory                = var.services_container_memories[local.service_name_admin_frontend]
  desired_count                   = var.services_desired_counts[local.service_name_admin_frontend]
  container_environment_variables = local.admin_frontend_env_vars
  ecs_cluster_arn                 = aws_ecs_cluster.dmp.arn
  ecs_execution_role_arn          = aws_iam_role.ecs_execution_role.arn
  environment_name                = var.environment_name
  lb_target_group_arn             = aws_lb_target_group.admin_frontend.arn
  ...
  service_name       = local.service_name_admin_frontend
...
resource "aws_lb_target_group" "admin_frontend" {
  name            = "${var.environment_name}-admin-frontend"
