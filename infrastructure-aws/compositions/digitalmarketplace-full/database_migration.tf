...
resource "aws_iam_policy" "write_migration_logs" {
  name   = "${var.project_name}-${var.environment_name}-db-migration-logs-write"
  policy = module.migration_log_group.write_log_group_policy_document_json
}
...
resource "aws_iam_role_policy_attachment" "db_migration_task__write_migration_logs" {
  ...
  policy_arn = aws_iam_policy.write_migration_logs.arn
}
