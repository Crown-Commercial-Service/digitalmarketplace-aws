module "gds-audit-cloudtrail" {
  ...
  s3_bucket_key_prefix = "${var.aws_prod_account_id}"              // As per RE guidelines this should be the id of the account exporting the CloudTrail trail log files
}
...
module "cloudtrail" {
  ...
  s3_bucket_key_prefix       = "${var.aws_prod_account_id}"
